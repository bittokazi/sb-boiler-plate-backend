...
public class DataLoaderTask implements CustomTaskChange {
	...
	public void execute(Database database) throws CustomChangeException {
		...
		try {
			...
			long roleId = 1L;
			for (String role : Arrays.asList(ROLE_ + Constants.getInstance().ROLE_SUPER_ADMIN, ROLE_ + Constants.getInstance().ROLE_ADMIN,
			...
			for (CSVRecord record : records) {
				String sql = "INSERT INTO users (id,first_name,last_name,password,email,enabled,change_password) VALUES(?,?,?,?,?,?,?)";

				PreparedStatement statement = databaseConnection.prepareStatement(sql);
				statement.setLong(1, userId);
				statement.setString(2, record.get("firstName"));
				statement.setString(3, record.get("lastName"));
				statement.setString(4, BCrypt.hashpw(record.get("password"), BCrypt.gensalt()));
				statement.setString(5, record.get("email"));
